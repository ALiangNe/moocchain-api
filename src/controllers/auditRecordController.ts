...
export async function getAuditRecordListController(req: AuthRequest, res: Response) {
  ...
  if (userRole === ROLE_STUDENT) {
    ...
    // 管理员可以按条件筛选
    if (targetId) params.targetId = parseInt(targetId as string);
    if (targetType) params.targetType = parseInt(targetType as string);
    if (auditType) params.auditType = parseInt(auditType as string);
    if (auditStatus !== undefined) params.auditStatus = parseInt(auditStatus as string);
    if (auditorId) params.auditorId = parseInt(auditorId as string);
  }
