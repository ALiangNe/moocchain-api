...
import { createResourceAuditModel } from '../models/auditRecordModel';
 ...
 * 教师在自己创建的课程中创建资源
 */
export async function createResourceService(teacherId: number, data: Partial<ResourceInfo>): Promise<ResourceInfo> {
  // 检查教师是否存在
  ...
  if (resource.resourceId) {
    await createResourceAuditModel(resource.resourceId, teacherId);
  }
 ...
 * 教师更新自己创建的资源信息
 */
export async function updateResourceService(teacherId: number, resourceId: number, data: Partial<ResourceInfo>): Promise<ResourceInfo> {
  // 检查资源是否存在
